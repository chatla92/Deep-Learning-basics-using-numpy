...
def softmax_cross_entropy_loss(Z, Y=np.array([])):
    ...
    if Y.size==0:
        ...
        #A = A+1e-15
        loss = -np.log(A[Y.astype(int), np.arange(Y.shape[1])]).sum(axis=0).mean()
    return A, cache, loss
...
def softmax_cross_entropy_loss_der(Y, cache):
    ...
    ### CODE HERE
    dZ = cache["A"]
    dZ[[Y.astype(int), np.arange(Y.shape[1])]] -= 1
    dZ = dZ/float(Y.shape[1])
    return dZ
...
def initialize_multilayer_weights(net_dims):
    ...
    for l in range(numLayers-1):
        parameters["W"+str(l+1)] = np.random.random((net_dims[l+1], net_dims[l])) * 0.01
        parameters["b"+str(l+1)] = np.random.random((net_dims[l+1], 1)) * 0.01
    return parameters
...
def classify(X, parameters):
    ...
    A_fin, _, _ = softmax_cross_entropy_loss(AL)
...
def multi_layer_network(X, Y, net_dims, num_iterations=500, learning_rate=0.2, decay_rate=0.01):
    ...
    for ii in range(num_iterations):
        ...
        A_fin, cache, cost = softmax_cross_entropy_loss(AL, Y)
...
def main():
    ...
    costs, parameters = multi_layer_network(train_data, train_label, net_dims, \
            num_iterations=num_iterations, learning_rate=learning_rate,decay_rate=0.0001)
    ...
    plt.ylabel('Cost')
    plt.title('Mini Project2:1.2 Multi-layer neural network for multi-class classifier(Two-layer)-Cost vs Iterations')
    plt.legend()
    plt.show()
